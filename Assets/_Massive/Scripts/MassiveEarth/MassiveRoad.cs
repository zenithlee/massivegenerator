...
namespace _Massive
{
  ...
  public class MassiveRoad : MonoBehaviour
  {
    ...
    public string[] AllowKinds = { "highway", "major_road", "minor_road" };
    ...
    public void Generate(string sData, DVector3 PositionMeters, string AssetPath)
    {
      ...
      foreach (IDictionary feature in features)
      {
        ...
        if (properties != null)
        {
          ...
          {
            kind = properties["kind"].ToString();
            if (properties["kind_detail"] != null)
            {
              kinddetail = properties["kind_detail"].ToString();
            }
          }
        }
    ...
    public void Run(List<List<Vector3>> segments, MeshData md)
    {
      if (segments.Count < 1)
        return;
      ...
      foreach (var roadSegment in segments)
      {
        ...
        for (int i = 1; i < count; i++)
        {
          ...
          if (i == 1)
          {
            norm = GetNormal(p1, p1, p2) * RoadWidth; //road width
          ...
          lastUv += dist;
          norm = GetNormal(p1, p2, p3) * RoadWidth;
        ...
        {
          newVerticeList[i] += new Vector3(0, GetMeshHeight(newVerticeList[i]), 0); // +1 for pop-through
        }
